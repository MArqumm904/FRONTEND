...
const EditCoverPhoto = ({
  ...
  useEffect(() => {
    console.log("on Cover pageId", pageId);
  }, [pageId]);
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      ...
      if (pageId) {
        ...
        response = await axios.delete(`${API_BASE_URL}/pages/${pageId}`, {
      ...
      if (fileToSend && fileToSend.size > 0) {
        ...
        if (pageId) {
          ...
          if (response.data && onCoverUpdate) {
            ...
            if (updatedGroup && updatedGroup.page_cover_photo) {
              const baseUrl = API_BASE_URL.replace("/api", "");
              const bannerUrl = updatedGroup.page_cover_photo.startsWith("http")
                ? updatedGroup.page_cover_photo
                : `${baseUrl}/storage/${updatedGroup.page_cover_photo}`;
